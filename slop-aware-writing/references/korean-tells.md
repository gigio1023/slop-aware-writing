...
C-8 판정 테스트: 부정절을 지웠을 때 주장이 그대로 남으면 수사적 대구다 — 지운다. 부정절을 지우면 무엇을 바로잡는 말인지 사라지면 재정의 문장이다 — 보존한다("X는 형식이 아니라 정책이다"처럼 기존 통념을 정정하는 진술). 측정 근거는 논설·블로그 산문 기준이라, 정의를 다시 세우는 기술 문서에서는 같은 표면형이 정보를 나른다. 같은 틀이 반복돼도 각각 실제 오해를 바로잡는다면 보존한다. 반복 때문에 요점이 가려질 때만 해당 문장을 고친다.
...
문두 접속사("또한/따라서/즉/나아가") 반복은 논리를 더하지 않으면 삭제(S1). "하지만/그러나"가 반복되면 실제 대조 관계를 점검하고, 없는 대조를 연출하는 표현만 삭제(S2). 메타 진입("이는/이 점에서") 반복은 본문에 녹인다(S1).
